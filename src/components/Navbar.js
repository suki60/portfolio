...
const Menu = ({ open }) => {
  return (
    <nav
      className={`flex transform flex-col justify-center bg-green-100 ${
        open ? 'translate-x-0' : '-translate-x-full'
      } absolute top-0 left-0 h-screen px-8 py-8 text-left transition-transform duration-300 ${open ? 'z-50' : '-z-1'}`}
    >
      <Link href='/'>home</Link>
...
const Burger = ({ open, setOpen }) => {
  return (
    <button className='z-50 flex h-8 w-8 flex-col justify-around p-0' onClick={() => setOpen(!open)}>
      <div
        className={`h-1 w-full origin-center transform rounded-full bg-gray-800 transition-all duration-300 ${
          open ? ' rotate-45' : 'rotate-0'
        }`}
        style={{ transformOrigin: '1px center' }}
      />
      <div
        className={`h-1 w-full transform rounded-full bg-gray-800 transition-all duration-300 ${
